...
import {
  createBirdHandler,
  getBirdsHandler,
  IMPOSIBLE_FIND_BIRD_TO_CREATE_IT_ERROR_MESSAGE,
...
describe(`${BIRDS_API_ROUTE}`, () => {
  ...
  test("should responds 200 when GET to get birds", async () => {
    jest
      .spyOn(BirdModel, "find")
      .mockImplementationOnce((): Promise<Bird[]> => Promise.resolve(birdsMock))
  ...
  test("should responds 201 when POST with correct params to create a bird", async () => {
    jest
      .spyOn(BirdModel, "create")
      .mockImplementationOnce((): Promise<Bird> => Promise.resolve(birdMock))
  ...
  test("should responds with a 400 error when POST with no params to create a bird", async () => {
    jest
      .spyOn(BirdModel, "create")
      .mockImplementationOnce((): Promise<Bird> => Promise.resolve(birdMock))
